...
class GAT_DGL_Custom(nn.Module):
    def __init__(
        ...
        self.gat_layers.append(self.conv(in_feats=in_channels, out_feats=hidden_node_channels_per_head[0], num_heads=heads[0], feat_drop=dropout, attn_drop=dropout, activation=F.elu))

        for i in range(1, num_layers-1):
            self.gat_layers.append(self.conv(in_feats=hidden_node_channels_per_head[i-1]*heads[i-1], out_feats=hidden_node_channels_per_head[i], num_heads=heads[i], feat_drop=dropout, attn_drop=dropout, activation=None))


        self.gat_layers.append(self.conv(in_feats=hidden_node_channels_per_head[-1]*heads[-1], out_feats=out_channels, num_heads=output_heads, feat_drop=dropout))
